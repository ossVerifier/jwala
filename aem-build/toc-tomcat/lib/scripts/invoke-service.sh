...
if [ "$JVMINST" = "1060" ]; then
    echo Service $1 not installed on server, continuing with invoke >> ~/LOG_invoke.txt
else
    echo Service $1 already exists >> ~/LOG_invoke.txt
    exit $STP_EXIT_CODE_FAILED
fi
cygstart $2/$1/bin/invoke.bat 
for (( c=1; c<=5; c++ ))
do
    /usr/bin/sleep 1
done
...
export JVMINST=`sc queryex $1 | head -1 | awk '{ sub(/:/,"",$4); print $4 }'`
if [ "$JVMINST" = "1060" ]; then
    echo Failed to install service $1 >> ~/LOG_invoke.txt
    exit $STP_EXIT_CODE_FAILED
fi
echo Invoke of service $1 was successful >> ~/LOG_invoke.txt
