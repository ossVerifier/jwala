...
SSLCertificateFile ../data/security/id/${webServerName}.cer
SSLCertificateKeyFile ../data/security/id/${webServerName}.key
...
# Do check expiration, because we need to have a certificate maintenance plan in place
...
# Do not check names and CN so that we can use the same cert across the server farm
...
<% apps.each() { %>
JkMount /jk${it.mount.replaceAll(" ", "")} lb-${it.name.replaceAll(" ", "")}
<% } %>
...
%>
BalancerMember https://${hostName}:${it.sslPort}${ctxPath} route=${jvmName}
<%  } %>
</Proxy>
<% } %>
