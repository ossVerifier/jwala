...
public class JvmServiceImplVerifyTest extends VerificationBehaviorSupport {
    ...
    public static void init() {
        System.setProperty(ApplicationProperties.PROPERTIES_ROOT_PATH,
                JvmServiceImplVerifyTest.class.getClassLoader().getResource("vars.properties").getPath()
    ...
    @Before
    public void setup() {
        reset(Config.mockJvmPersistenceService, Config.mockGroupService, Config.mockApplicationService, Config.mockFileManager,
    ...
    public void testCreateValidateAdd() {
        ...
        for (final AddJvmToGroupRequest addCommand : addCommands) {
            verify(Config.mockGroupService, times(1)).addJvmToGroup(matchCommand(addCommand),
                    eq(Config.mockUser));
        }
    }
    ...
    public void testCreateValidateInheritsDefaultTemplatesJvmTemplateThrowsIOException() throws IOException {
        ...
        when(Config.mockGroupService.getGroupJvmsResourceTemplateNames(anyString())).thenReturn(templateNames);
        when(Config.mockGroupService.getGroupJvmResourceTemplate(anyString(), anyString(), any(ResourceGroup.class), anyBoolean())).thenReturn("<server>xml</server>");
        when(Config.mockGroupService.getGroupJvmResourceTemplateMetaData(anyString(), anyString())).thenReturn("{deployPath:c:/fake/path}");
    ...
    public void testCreateValidateInheritsDefaultTemplatesAppTemplateThrowsIOException() {
        ...
        when(Config.mockGroupService.getGroupJvmsResourceTemplateNames(anyString())).thenReturn(templateNames);
        when(Config.mockGroupService.getGroupAppsResourceTemplateNames(anyString())).thenReturn(appTemplateNames);
        when(Config.mockGroupService.getGroupAppResourceTemplateMetaData(anyString(), anyString())).thenReturn("{deployPath:c:/fake/app/path}");
    ...
    public void testUpdateJvmShouldValidateCommand() {
        ...
        jvmService.updateJvm(updateJvmRequest, Config.mockUser);

        verify(updateJvmRequest, times(1)).validate();
        verify(Config.mockJvmPersistenceService, times(1)).updateJvm(updateJvmRequest);
        verify(Config.mockJvmPersistenceService, times(1)).removeJvmFromGroups(Matchers.<Identifier<Jvm>>anyObject());
        for (final AddJvmToGroupRequest addCommand : addCommands) {
            verify(Config.mockGroupService, times(1)).addJvmToGroup(matchCommand(addCommand), eq(Config.mockUser));
        }
    }
    ...
    public void testDeleteJvmWindowsServiceForNonExistentService() {
        ...
        when(Config.mockJvmControlService.controlJvm(eq(controlJvmRequest), any(User.class))).thenReturn(commandOutput);

        jvmService.deleteJvmWindowsService(controlJvmRequest, mockJvm, Config.mockUser);
        verify(Config.mockJvmControlService).controlJvm(eq(controlJvmRequest), eq(Config.mockUser));
    }
    ...
    public void testGenerateConfig() throws IOException {
        ...
        when(Config.mockJvmPersistenceService.findJvmByExactName(eq(jvm.getJvmName()))).thenReturn(jvm);
        final String templateContent = "<server>test</server>";
        when(Config.mockJvmPersistenceService.getJvmTemplate(eq("server.xml"), eq(jvm.getId()))).thenReturn(templateContent);
        when(Config.mockResourceService.generateResourceGroup()).thenReturn(new ResourceGroup());
        when(Config.mockResourceService.generateResourceFile(anyString(), anyString(), any(ResourceGroup.class), any(Jvm.class), any(ResourceGeneratorType.class))).thenReturn(templateContent);
    ...
    public void testGenerateThrowsExceptionForEmptyTemplate() {
        final Jvm jvm = new Jvm(new Identifier<Jvm>(-123456L),
                ...
                "EXAMPLE_OPTS=%someEnv%/someVal", JvmState.JVM_STOPPED, null, null, null, null, null);
        when(Config.mockJvmPersistenceService.findJvmByExactName(eq(jvm.getJvmName()))).thenReturn(jvm);
        when(Config.mockJvmPersistenceService.getJvmTemplate(eq("server.xml"), eq(jvm.getId()))).thenReturn("");
    ...
    public void testGenerateServerXmlConfig() {
        ...
        when(Config.mockJvmPersistenceService.findJvmByExactName(testJvmName)).thenReturn(testJvm);
        String expectedValue = "<server>xml-content</server>";
        when(Config.mockResourceService.generateResourceGroup()).thenReturn(new ResourceGroup());
        when(Config.mockResourceService.generateResourceFile(anyString(), eq(expectedValue), any(ResourceGroup.class), eq(testJvm), any(ResourceGeneratorType.class))).thenReturn(expectedValue);
        when(Config.mockJvmPersistenceService.getJvmTemplate(anyString(), any(Identifier.class))).thenReturn(expectedValue);
    ...
    public void testGenerateContextXmlConfig() {
        ...
        when(Config.mockJvmPersistenceService.findJvmByExactName(testJvmName)).thenReturn(jvm);
        String expectedValue = "<server>xml-content</server>";
        when(Config.mockJvmPersistenceService.getJvmTemplate(anyString(), any(Identifier.class))).thenReturn(expectedValue);
        when(Config.mockResourceService.generateResourceGroup()).thenReturn(new ResourceGroup());
        when(Config.mockResourceService.generateResourceFile(anyString(), eq(expectedValue), any(ResourceGroup.class), eq(jvm), any(ResourceGeneratorType.class))).thenReturn(expectedValue);
    ...
    public void testGenerateInvokeBat() {
        ...
        when(Config.mockFileManager.getResourceTypeTemplate(anyString())).thenReturn(expectedValue);
        when(Config.mockResourceService.generateResourceGroup()).thenReturn(new ResourceGroup());
        when(Config.mockResourceService.generateResourceFile(anyString(), anyString(), any(ResourceGroup.class), eq(jvm), any(ResourceGeneratorType.class))).thenReturn(expectedValue);
    ...
    public void testGenerateAndDeployConfig() throws CommandFailureException, IOException {
        ...
        when(Config.mockJvmControlService.controlJvm(eq(new ControlJvmRequest(mockJvm.getId(), JvmControlOperation.INVOKE_SERVICE)), any(User.class))).thenReturn(commandOutput);

        when(Config.mockJvmPersistenceService.getResourceTemplateMetaData(anyString(), anyString())).thenReturn("{\"deployFileName\":\"server-test-deploy-config.xml\", \"deployPath\":\"c:/fake/test/path\"}");
        ...
        when(Config.mockJvmControlService.controlJvm(eq(new ControlJvmRequest(mockJvm.getId(), JvmControlOperation.INVOKE_SERVICE)), any(User.class))).thenReturn(mockExecDataFail);
...
//        FileUtils.deleteDirectory(new File("./" + jvm.getJvmName() + "null"));
    ...
    public void testGenerateAndDeployJVMFailsDeployingInvokeService() throws CommandFailureException {
        ...
        when(Config.mockJvmControlService.secureCopyFile(any(ControlJvmRequest.class), contains(AemControl.Properties.INVOKE_SERVICE_SCRIPT_NAME.getValue()), anyString(), anyString(), anyBoolean())).thenReturn(commandOutputFails);
        when(mockJvm.getId()).thenReturn(new Identifier<Jvm>(111L));
        when(mockJvm.getState()).thenReturn(JvmState.JVM_STOPPED);
...
*/
    ...
    public void testGenerateAndDeployFile() throws CommandFailureException, IOException {
        ...
        when(Config.mockJvmPersistenceService.getResourceTemplateMetaData(anyString(), anyString())).thenReturn("{\"deployFileName\":\"server.xml\", \"deployPath\":\"/\",\"contentType\":\"text/plain\"}");
...
//        FileUtils.deleteDirectory(new File("./" + jvm.getJvmName()));
    ...
    static class Config {
        ...
        static FileManager mockFileManager = mock(FileManager.class);
        ...
        @Bean
        public FileManager getMockFileManager() {
        ...
        public JvmService getJvmService() {
            return new JvmServiceImpl(mockJvmPersistenceService, mockGroupService, mockApplicationService, mockFileManager,
