...
CREATE TABLE jvm (id INTEGER NOT NULL IDENTITY, createBy VARCHAR(255), createDate TIMESTAMP, lastUpdateDate TIMESTAMP, name VARCHAR(255) NOT NULL, updateBy VARCHAR(255), ajpport integer not null, hostName VARCHAR(255), httpport integer not null, httpsport integer not null, redirectport integer not null, shutdownport integer not null, CONSTRAINT U_JVM_NAME UNIQUE (name));
...
CREATE TABLE webserver (id INTEGER NOT NULL IDENTITY, createBy VARCHAR(255), createDate TIMESTAMP, lastUpdateDate TIMESTAMP, name VARCHAR(255) NOT NULL, updateBy VARCHAR(255), host VARCHAR(255), port INTEGER, httpsport INTEGER, CONSTRAINT U_WBSRVER_NAME UNIQUE (name));
...
CREATE TABLE CURRENT_JVM_STATE (ID INTEGER NOT NULL IDENTITY, AS_OF TIMESTAMP not null, STATE VARCHAR(255));
CREATE TABLE webserver_control_history (id INTEGER NOT NULL IDENTITY, completedDate TIMESTAMP, controlOperation VARCHAR(255), webserverId BIGINT, requestedBy VARCHAR(255), requestedDate TIMESTAMP, returnCode INTEGER, returnErrorOutput VARCHAR(2048), returnOutput VARCHAR(2048));
