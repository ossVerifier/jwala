...
public class TocHttpClient {
    ...
    /**
      ...
      */
    def doMultipartPostCheck201(String url, File file) {
      ...
      if(response.getStatusLine().statusCode != 201) {
          println "throwing RestException " + response.getStatusLine().statusCode
          throw new RestException("WAR upload did not succeed", responseString)
      }

      println "Got a 201 still good"
      return [response, responseString];
    }

    /**
     * @return Slurped applicationResponseContent
     */
    public def multipartPostReturningJson(String url, File file) {
        def response
		def responseString
		(response, responseString) = doMultipartPostCheck201(url, file);
    ...
    /**
    * @return 'Success' on success. Exception otherwise.
    */
    public def multipartPostForFileUpload(String url, File file) {

	    def response
		def responseString
		(response, responseString) = doMultipartPostCheck201(url, file);

		int idx = responseString.indexOf("\"message\":\"SUCCESS\"");
