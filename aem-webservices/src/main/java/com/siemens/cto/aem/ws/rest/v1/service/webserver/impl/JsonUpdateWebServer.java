...
public class JsonUpdateWebServer {
    ...
    public JsonUpdateWebServer(final String aWebServerId, final String aWebServerName, final String aHostName,
        ...
        hostName = aHostName;
        portNumber = Integer.valueOf(aPortNumber);
        webServerId = aWebServerId;
    }

    public void addGroupId(final String gid) {
        groupIds.add(gid);
    }
    ...
    public UpdateWebServerCommand toUpdateWebServerCommand() throws BadRequestException {
        ...
        try {
            final List<Identifier<Group>> groups = new ArrayList<>(groupIds.size());
            for (final String grp : groupIds) {
                groups.add(Identifier.id(Long.parseLong(grp), Group.class));
            }
            return new UpdateWebServerCommand(new Identifier<WebServer>(webServerId), groups, webserverName, hostName,
                    portNumber);
    ...
    static class JsonUpdateWebServerDeserializer extends AbstractJsonDeserializer<JsonUpdateWebServer> {
        ...
        public JsonUpdateWebServer deserialize(final JsonParser jp, final DeserializationContext ctxt)
            ...
            final JsonUpdateWebServer juws =
                    new JsonUpdateWebServer(node.get("webserverId").getValueAsText(), node.get("webserverName")
                            .getTextValue(), node.get("hostName").getTextValue(), node.get("portNumber")
                            .getValueAsText());
    ...
    public boolean equals(final Object obj) {
        ...
        final JsonUpdateWebServer other = (JsonUpdateWebServer) obj;
        if (groupIds == null) {
